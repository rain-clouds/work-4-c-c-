...
public:
    void hexagonRec(double lungime, int nivel, CPunct& p, CVector& v, int d)
    {
        ...
        if (nivel == 1)
        {//la pasul 1, deseneaza jumatate de hexagon
            //aceasta jumatate va fi copiata in pasul 2 si asa mai departe
        ...
        { //deseneaza vectorul dela pasul anterior.
            //in stanga si in dreapta vectorului, concatenate de acesta, vor fi copii ale acestuia rotite
            CVector copievect;
            v.rotatie(-d * 60);
            copievect = v;
...
public:
    ...
    void modelPatrat(double lungime, CPunct p, CVector v)
    {//modelul de baza dupa care se deseneaza un patrat
        //acest patrat ne va folosi atat cand desenam patratul principal cu chenarul din jur, dar si cand facem recursia
        int i;//unghiul de rotatie
    ...
    void recursia(double lungime, int nivel, CPunct p, CVector v)
    {
        if (nivel == 0)
        {

            modelPatrat(lungime, p, v);//daca nivelul e 0, vom desena doar un patrat mare intr un chenar
        }
        else
        {
            recursia(lungime, nivel - 1, p, v);//pt afisarea pe ecran a tuturor nivelelor
            //ce sa intampla pe un nivel
            lungime = lungime / 3;
            int ii;
            ...
            for ( ii = 0; ii < 9; ii++)
            {
                ...
                recursia(lungime, nivel - 1, temp, v);
    ...
    void afisare(int nivel)
    {
        ...
        //Chenarul
        modelPatrat(lungime, p, v);

        double x, y;
        p.getxy(x, y);
        p = CPunct(x / 3, y / 3);

        recursia(lungime / 3, nivel, p, v);
    }
...
class Mandelbrot {
public:
    Mandelbrot() : nr_max_iter(NRITER_JF), limit(2) {};

    int apartine(CComplex& c) {
    ...
    void setNrMaxIter(int v) { if(v <= NRITER_JF); nr_max_iter = v; }
