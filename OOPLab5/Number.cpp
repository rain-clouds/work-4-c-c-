...
Number::Number(const char* value, int base)
{
	...
	this->base = base;
	negative = value[0] == '-';// verificam daca primul caracter este -
}
...
void Number::SwitchBase(int newBase) {
	const char* valoare = this->value;
	long long n = convertToBaseTen(valoare, base);//trecem in baza 10
	char* newValue = convertToBase(n, newBase);//si din baza 10 in baza ceruta
...
long long Number::getBaseTen() {
	...
	if (this->negative)  return ((-1) *n);
		 return n;
}


long long Number::convertToBaseTen(const char* s, int base) {

	long long n = 0, p = 1;

	for (int i = strlen(value) - 1; i >= (int)negative; i--) {
		n =n+ normalizeDigit(s[i]) * p;
		p=p* base;
	}
